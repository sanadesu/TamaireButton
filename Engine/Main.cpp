	...
	while (msg.message != WM_QUIT)
	{
		...
		else
		{
			...
			if ((nowTime - lastUpdateTime) * fpsLimit > 1000.0f)
			{
				...
				switch (ScreenSplit::GetScreenSplit())
				{
					case 1:
						pPlayer = (Player*)pRootObject->FindObject("Player");
					...
					//右画面描画
					{
						pPlayer = (Player*)pRootObject->FindObject("Player2");
					...
					//左下画面描画
					{
						pPlayer = (Player*)pRootObject->FindObject("Player3");

						Direct3D::SetViewPort(5);
						...
						//右下画面描画
					{
						pPlayer = (Player*)pRootObject->FindObject("Player4");
