...
namespace Direct3D
{
	...
	D3D11_VIEWPORT vp_leftUp;
	D3D11_VIEWPORT vp_rightUp;
	D3D11_VIEWPORT vp_leftDown;
	D3D11_VIEWPORT vp_rightDown;
	...
	HRESULT Direct3D::Initialize(HWND hWnd, int screenWidth, int screenHeight)
	{
		...
		////左上画面用
		{
			vp_leftUp.Width = (float)screenWidth / 2;			//幅
			vp_leftUp.Height = (float)screenHeight / 2;		//高さ
			vp_leftUp.MinDepth = 0.0f;		//手前
			vp_leftUp.MaxDepth = 1.0f;		//奥
			vp_leftUp.TopLeftX = 0;		//左
			vp_leftUp.TopLeftY = 0;		//上
		}

		//右上画面用
		{
			vp_rightUp.Width = (float)screenWidth / 2;			//幅
			vp_rightUp.Height = (float)screenHeight / 2;		//高さ
			vp_rightUp.MinDepth = 0.0f;		//手前
			vp_rightUp.MaxDepth = 1.0f;		//奥
			vp_rightUp.TopLeftX = (float)screenWidth / 2;		//左
			vp_rightUp.TopLeftY = 0;		//上
		}

		////左下画面用
		{
			vp_leftDown.Width = (float)screenWidth / 2;			//幅
			vp_leftDown.Height = (float)screenHeight / 2;		//高さ
			vp_leftDown.MinDepth = 0.0f;		//手前
			vp_leftDown.MaxDepth = 1.0f;		//奥
			vp_leftDown.TopLeftX = 0;		//左
			vp_leftDown.TopLeftY = (float)screenHeight / 2;		//上
		}

		//右下画面用
		{
			vp_rightDown.Width = (float)screenWidth / 2;			//幅
			vp_rightDown.Height = (float)screenHeight / 2;		//高さ
			vp_rightDown.MinDepth = 0.0f;		//手前
			vp_rightDown.MaxDepth = 1.0f;		//奥
			vp_rightDown.TopLeftX = (float)screenWidth / 2;		//左
			vp_rightDown.TopLeftY = (float)screenHeight / 2;		//上
		}
	...
	void InitShaderBundle()
	{
		...
		//Toon
		{
			...
			D3DCompileFromFile(L"Shader/Toon.hlsl", nullptr, nullptr, "VS", "vs_5_0", NULL, 0, &pCompileVS, NULL);
			pDevice_->CreateVertexShader(pCompileVS->GetBufferPointer(), pCompileVS->GetBufferSize(), NULL, &shaderBundle[SHADER_TOON].pVertexShader);


			// ピクセルシェーダの作成（コンパイル）
			ID3DBlob *pCompilePS = NULL;
			D3DCompileFromFile(L"Shader/Toon.hlsl", nullptr, nullptr, "PS", "ps_5_0", NULL, 0, &pCompilePS, NULL);
			pDevice_->CreatePixelShader(pCompilePS->GetBufferPointer(), pCompilePS->GetBufferSize(), NULL, &shaderBundle[SHADER_TOON].pPixelShader);
			...
			pDevice_->CreateInputLayout(layout, sizeof(layout) / sizeof(D3D11_INPUT_ELEMENT_DESC), pCompileVS->GetBufferPointer(), pCompileVS->GetBufferSize(), &shaderBundle[SHADER_TOON].pVertexLayout);
			...
			pDevice_->CreateRasterizerState(&rdc, &shaderBundle[SHADER_TOON].pRasterizerState);
		}
		...
		//3D
		{
			...
			pDevice_->CreateVertexShader(pCompileVS->GetBufferPointer(), pCompileVS->GetBufferSize(), NULL, &shaderBundle[SHADER_NORMAL].pVertexShader);


			// ピクセルシェーダの作成（コンパイル）
			ID3DBlob* pCompilePS = NULL;
			D3DCompileFromFile(L"Shader/Simple3D.hlsl", nullptr, nullptr, "PS", "ps_5_0", NULL, 0, &pCompilePS, NULL);
			pDevice_->CreatePixelShader(pCompilePS->GetBufferPointer(), pCompilePS->GetBufferSize(), NULL, &shaderBundle[SHADER_NORMAL].pPixelShader);
			...
			pDevice_->CreateInputLayout(layout, 3, pCompileVS->GetBufferPointer(), pCompileVS->GetBufferSize(), &shaderBundle[SHADER_NORMAL].pVertexLayout);
			...
			pDevice_->CreateRasterizerState(&rdc, &shaderBundle[SHADER_NORMAL].pRasterizerState);
		}
		...
		//	// ピクセルシェーダの作成（コンパイル）
		//	ID3DBlob* pCompilePS = NULL;
		//	D3DCompileFromFile(L"Shader/Circle.hlsl", nullptr, nullptr, "PS", "ps_5_0", NULL, 0, &pCompilePS, NULL);
		//	pDevice_->CreatePixelShader(pCompilePS->GetBufferPointer(), pCompilePS->GetBufferSize(), NULL, &shaderBundle[SHADER_CIRCLE].pPixelShader);
		...
		//		{ "TEXCOORD", 0, DXGI_FORMAT_R32G32_FLOAT,    0, vectorSize * 1, D3D11_INPUT_PER_VERTEX_DATA, 0 },
		//	};
		//	//pDevice_->CreateInputLayout(layout, 2, pCompileVS->GetBufferPointer(), pCompileVS->GetBufferSize(), &shaderBundle[SHADER_CIRCLE].pVertexLayout);
		...
		//	rdc.FillMode = D3D11_FILL_SOLID;
		//	rdc.FrontCounterClockwise = TRUE;
		//	pDevice_->CreateRasterizerState(&rdc, &shaderBundle[SHADER_CIRCLE].pRasterizerState);
		//}
	...
	void SetViewPort(int lr_)
	{
		...
		case 2:
			...
			break;
		case 3:
			pContext_->RSSetViewports(1, &vp_leftUp);
			break;
		case 4:
			pContext_->RSSetViewports(1, &vp_rightUp);
			break;
		case 5:
			pContext_->RSSetViewports(1, &vp_leftDown);
			break;
		case 6:
			pContext_->RSSetViewports(1, &vp_rightDown);
			break;
		}
