...
void Result::Initialize()
{
    ...
    {
        //使用する画像ファイル名
        const char* fileName[] = { "WhiteResult.png", "RedResult.png","Result.png", "RedWin.png", "Draw.png", "Number.png", "Number.png" };
        ...
        //使用するサウンドファイル名
        const char* soundFileName[] = { "End.wav", "Drum.wav","Result.wav", "Move.wav" };

        //ロード
        for (int i = 0; i < Sound::SOUND_MAX; i++)
        {
            hSound_[i] = Audio::Load(soundFileName[i], false, 1, 10);
            assert(hSound_[i] >= 0);
        }
        hSound_[Sound::SOUND_DRUM] = Audio::Load(soundFileName[Sound::SOUND_DRUM], true, 1, 10);
